...
util plot error-table -r dft_ -p ${gap_name}_ --kw "param_filename=${gap_filename}" -o ${evaled_train} --num_inputs_per_python_subprocess 50 ${train_filename}  > err_${gap_name}_${train_name}.txt
util plot error-table -r dft_ -p ${gap_name}_ --kw "param_filename=${gap_filename}" -o ${evaled_test}  --num_inputs_per_python_subprocess 50 ${test_filename} > err_${gap_name}_${test_name}.txt
