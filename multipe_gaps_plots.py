    ...
    for sym, sym_dict in my_dict.items():
        for config_type, values in sym_dict.items():
            try:
                ...
                force_dict[config_type].append(vaules)

    # TODO check how are you array-ing thie stuff
    for config_type, values in force_dict.items():
        force_dict[config_type] = np.array(values)

    return force_dict

def get_rmse_dict(obs, dft_data, gap_data):
    rmses = dict()
    rmses['Combined'] = util.get_rmse(util.dict_to_vals(dft_data[obs]), util.dict_to_vals(gap_data[obs]))
...
def plot_heatmap(data_dict, ax, obs):
    ...
    if obs == 'Energy':
        ...
        units = 'eV/Å '
    cbar.ax.set_ylabel(f'{obs} RMSE, {units}', rotation=90, labelpad=20)
    plt.title(f'{obs} RMSE', fontsize=14 )
...
def rmse_plots(train_filename, gaps_dir, output_dir=None, prefix=None):
    ...
    for gap_fname in gap_fnames:
        ...
        F_rmses[gap_title] = get_rmse_dict(obs='forces', dft_data=dft_data, gap_dtaa=gap_data)

    # Make plot
    # TODO make automatic scaling
    # TODO save the pandas dataframe somewhere somehow

    plt.figure(fig_size=(25, 10))
