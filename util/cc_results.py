...
def calculate(calculation_stem, compound_wdir,
              orca_template_fname, sub_template_fname,
              mult, fname, submit):


    make_move_calc_files(calculation_stem,
                         orca_template_fname,
                         sub_template_fname,
                         mult,
                         fname)
    ...
    if submit:
        subprocess.run(f'qsub sub.sh', shell=True)
    ...
    for fname in xyz_filenames:
        ...
        for mult in multiplicities:
            for orca_template_fname, method in zip(template_fnames, methods):
                ...
                if task == 'calculate':
                    calculate(calculation_stem, compound_wdir,
                              orca_template_fname, sub_template_fname,
                              mult, fname, submit)
...
def plot_densities(calculation_stem, method):
    ...
    if method=='uks_cc-pvdz':
        shutil.move(f'{calculation_stem}.eldens.cube',
                    f'{calculation_stem}.uks_scf.eldens.cube')
        shutil.move(f'{calculation_stem}.spindens.cube',
                    f'{calculation_stem}.uks_scf.spindens.cube')

    elif method == 'dlpno-ccsd_cc-pvdz':

        shutil.move(f'{calculation_stem}.eldens.cube',
                    f'{calculation_stem}.uhf_scf.eldens.cube')
        shutil.move(f'{calculation_stem}.spindens.cube',
        ...
        shutil.move(f'{calculation_stem}.eldens.cube',
                    f'{calculation_stem}.mdci.eldens.cube')
        shutil.move(f'{calculation_stem}.spindens.cube',
...
def make_move_calc_files(calculation_stem,
                         orca_template_fname,
                         sub_template_fname,
                         multiplicity,
                         xyz_fname):
    ...
    make_orca_input_file(orca_template_fname,
                         multiplicity,
                         input_xyz_fname,
                         calculation_stem)
...
def make_sub_file(sub_template_fname,
                  calculation_stem):
...
def make_orca_input_file(orca_template_fname, multiplicity, input_xyz_fname,
                   calculation_stem):
