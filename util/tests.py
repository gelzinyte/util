...
def dft_optimise_start_ats(names, output_dir, molpro_wdir):
    ...
    for idx, at_name in enumerate(names):
        # print(f'Making optg {idx} template')
        optg_template_fname = f'optg_template_{i}.txt'
        ...
        print(f'writing submission script {i}')
        submission_script_fname = f'dft_geo_opt_{i}.sh'
        job_name = f'dft_optg_{i}'
        output_fname =  f'opt_{i}.out'
        command = f'{mp_command} {optg_template_fname} -o {output_fname}'

        qm.write_generic_submission_script(
...
def gap_optg_test(gap_no, mp_template,  no_runs, fmax, steps, dft_stride, no_cores):
    ...
    for run_idx in range(no_runs):
        ...
        if not os.path.isfile(f'{traj_name}.xyz'):
            ...
            print(f'found gap trajectory: {traj_name}')
...
def do_dft_reevaluation(traj_name, dft_stride, mp_template, no_cores):
    ...
    for idx, dft_in_at, dft_out_at in zip(dft_at_idx, dft_atoms, dft_out_atoms):
        if dft_in_at.positions != dft_out_at.positions:
    ...
    write(f'{traj_name}_dft.xyz', dft_out_atoms, 'ext')
...
def plot_gap_optg_results(gap_no, no_runs):
    ...
    for idx in range(no_runs):
        ...
        ax1.plot(dt['idx_gap'], dt['es_gap'], label=label_gap)
        ax1.scatter(dt['idx_gap'], dt['fmax_gap'], marker='x', label=label_gap)

        ax2.plot(dt['idx_dft'], dt['es_dft'], label=label_dft)
        ax2.scatter(dt['idx_dft'], dt['fmax_dft'], marker='x', label=label_dft)
