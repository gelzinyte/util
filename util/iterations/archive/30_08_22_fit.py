    ...
    * is parallelisation done correctly
    ...
    for cycle_idx in range(0, num_cycles + 1):
        ...
        if not (tests_wdir / f"{pred_prop_prefix}on_{train_set_fname.name}").exists():
            # it.check_dft(train_set_fname, "dft_", orca_calc, tests_wdir)
        ...
        orca_kwargs["workdir_root"] = cycle_dir / "orca_wdir_on_extra_train"
        orca_kwargs["keep_files"] = "default"
        inputs = generic.run(
            inputs=inputs, 
            outputs=outputs,
            calculator=(orca.ORCA, [], orca_kwargs),
