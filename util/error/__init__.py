...
def process(all_atoms, ref_energy_key, pred_energy_key,
             ...
             pred_forces_key=None, isolated_atoms=None, skip=False):
...
def check_skip(at, ref_energy_key, pred_energy_key, ref_forces_key, pred_forces_key, skip):
    if ref_energy_key not in at.info or pred_energy_key not in at.info:   
        if skip:
            return True 
        else:
            print(at.info)
    ...
    if ref_forces_key is not None and pred_forces_key is not None: 
        if ref_forces_key not in at.arrays or pred_forces_key not in at.arrays:   
            if skip:
                return True 
            else:
                print(at.info)
